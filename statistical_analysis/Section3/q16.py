'''
Analyzes data for Question 16: "How concerning are the following API Runtime Protection challenges for your organization?".

This script processes survey responses where organizations rate their concern levels
for specific API Runtime Protection challenges (e.g., 'Data Leakage', 'API Security Attacks',
'Malicious user detection'). The ratings are assumed to be numeric.

It calculates:
- Overall statistics for each challenge's concern level using `calculate_stats`.
...
def analyze_q16(file_path: str) -> dict:
    """
    Analyzes concern levels for various API Runtime Protection challenges (Question 16).

    The function reads data from the specified Excel file. For each API Runtime Protection challenge
    ...
    # Columns representing different API Runtime Protection challenges to be rated
    concern_cols = [
        'Data Leakage',
        'Data Tampering',
    ...
    summary = {
        'question_text': '16 How concerning are the following API Runtime Protection challenges for your organization?',
...
if __name__ == "__main__":
    ...
    default_data_file = os.path.join(
        os.path.dirname(__file__), '..', '..', 'data', 'Section 3', '3.4 API Runtime', 
        '16 API Runtime- Concern.xlsx'
    )

    if not os.path.exists(default_data_file):
        print(f"Warning: Default data file not found at {default_data_file}")
        q16_results = {"error": f"Data file not found: {default_data_file}"}
    else:
        try:
            q16_results = analyze_q16(default_data_file)
        except Exception as e:
            print(f"An unexpected error occurred while analyzing {default_data_file}: {e}")
            q16_results = {"error": str(e)}
    
    print(json.dumps(q16_results, indent=2)) 