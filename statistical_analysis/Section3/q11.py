'''
Analyzes data for Question 11: "How effective are your current solutions in addressing the following API Discovery challenges within your organization?".

This script processes survey responses where organizations rate the effectiveness of their
current solutions for specific API Discovery challenges (e.g., 'Shadow/undocumented APIs', 'Zombie/dormant APIs').
...
def analyze_q11(file_path: str) -> dict:
    """
    Analyzes the effectiveness of current solutions for various API Discovery challenges (Question 11).
    ...
    # Columns representing different API Discovery challenges for which solution effectiveness is rated
    solution_cols = [
    ...
    summary = {
        ...
        'main_stats': {
            ...
            'solution_averages': solution_averages # Correctly use calculated averages
        }
    }
...
if __name__ == "__main__":
    ...
    default_data_file = os.path.join(
        ...
        '11 API Discovery & Mapping- Effectiveness of Current Solutions.xlsx'
    )

    if not os.path.exists(default_data_file):
        print(f"Warning: Default data file not found at {default_data_file}")
        q11_results = {"error": f"Data file not found: {default_data_file}"}
    else:
        try:
            q11_results = analyze_q11(default_data_file)
        except Exception as e:
            print(f"An unexpected error occurred while analyzing {default_data_file}: {e}")
            q11_results = {"error": str(e)}
    
    print(json.dumps(q11_results, indent=2)) 