'''
Analyzes data for Question 19: "How effective are your current solutions in identifying vulnerable and/or undocumented/shadow APIs?".

This script processes survey responses where organizations rate the effectiveness of their current solutions
for specific API Security Testing methods (e.g., 'Code-based discovery', 'Crawler-based discovery',
'Traffic-based discovery') in identifying vulnerable or shadow APIs. The ratings are assumed to be numeric.
...
def analyze_q19(file_path: str) -> dict:
    """
    Analyzes the effectiveness of current solutions for identifying vulnerable APIs (Question 19).

    The function reads data from the specified Excel file. For each API Security Testing method
    ...
    Demographic analysis is performed for all specified methods.
    ...
    Returns:
        dict: A dictionary containing the analysis summary, including:
              ...
              - 'main_stats': Contains 'solution_stats' (distribution of ratings for each method's effectiveness)
                              and 'solution_averages' (average effectiveness rating for each method).
    ...
    # Columns representing different API Security Testing methods for which solution effectiveness is rated
    solution_cols = [
...
if __name__ == "__main__":
    ...
    default_data_file = os.path.join(
        ...
        '19 API Security Testing- Effectiveness.xlsx'
    )

    if not os.path.exists(default_data_file):
        print(f"Warning: Default data file not found at {default_data_file}")
        q19_results = {"error": f"Data file not found: {default_data_file}"}
    else:
        try:
            q19_results = analyze_q19(default_data_file)
        except Exception as e:
            print(f"An unexpected error occurred while analyzing {default_data_file}: {e}")
            q19_results = {"error": str(e)}
    
    print(json.dumps(q19_results, indent=2)) 