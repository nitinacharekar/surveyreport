'''
Analyzes data for Question 18: "How concerning are the following API Security Testing challenges for your organization?".

This script processes survey responses where organizations rate their concern levels
for specific API Security Testing challenges (e.g., 'Code-based discovery', 'Crawler-based discovery',
'Traffic-based discovery'). The ratings are assumed to be numeric.
...
def analyze_q18(file_path: str) -> dict:
    """
    Analyzes concern levels for various API Security Testing challenges (Question 18).

    The function reads data from the specified Excel file. For each API Security Testing challenge
    ...
    # Columns representing different API Security Testing challenges to be rated
    concern_cols = [
...
if __name__ == "__main__":
    ...
    default_data_file = os.path.join(
        ...
        '18 API Security Testing- Concern.xlsx'
    )

    if not os.path.exists(default_data_file):
        print(f"Warning: Default data file not found at {default_data_file}")
        q18_results = {"error": f"Data file not found: {default_data_file}"}
    else:
        try:
            q18_results = analyze_q18(default_data_file)
        except Exception as e:
            print(f"An unexpected error occurred while analyzing {default_data_file}: {e}")
            q18_results = {"error": str(e)}
    
    print(json.dumps(q18_results, indent=2)) 