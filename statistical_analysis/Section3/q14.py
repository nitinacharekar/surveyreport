"""
Analyzes data for Question 14: "How concerning are the following API Access Control challenges for your organization?".

This script processes survey responses where organizations rate their concern levels
for specific API Access Control challenges (e.g., 'Complying with standard access control methods such as OAuth/OIDC',
'API access control for third-party partner companies'). The ratings are assumed to be numeric.
...
def analyze_q14(file_path: str) -> dict:
    """
    Analyzes concern levels for various API Access Control challenges (Question 14).
    ...
    # Columns representing different API Access Control challenges to be rated
    concern_cols = [
...
if __name__ == "__main__":
    ...
    default_data_file = os.path.join(
        ...
        '14 API Access Control- Concern.xlsx'
    )

    if not os.path.exists(default_data_file):
        print(f"Warning: Default data file not found at {default_data_file}")
        q14_results = {"error": f"Data file not found: {default_data_file}"}
    else:
        try:
            q14_results = analyze_q14(default_data_file)
        except Exception as e:
            print(f"An unexpected error occurred while analyzing {default_data_file}: {e}")
            q14_results = {"error": str(e)}
    
    print(json.dumps(q14_results, indent=2)) 